...
// add-cart 
/*
var qte = document.querySelectorAll('.add-cart')
var prix = document.querySelector('.price')

function calSum() {
    let sum = 180;
    for (var i = 0; i < qte.length - 1; i++) {
        price = parseInt(prix[i].children[0].innerHTML)
        sum += price
    }
...
function updatePrice(){
    var cartItemContainer = document.querySelector('main')[0]
    var cartRows = cartItemContainer.querySelector('.block-product')
    var total = price
    for (var i= 0; i< cartRows.length; i++) {
        var cartRow = cartRows[i]
        var priceElement = cartRow.querySelector('.price')[0]
        var quantityElement = cartRow.querySelector('.quantity')[0]
